...
bool eval_guard(const Guard& g, const T& x)
{
  return false;
}
...
template <typename T>
bool eval_guard(const std::initializer_list<T>& guard, const T& x)
{
  return (std::find(guard.begin(),guard.end(),x)!=guard.end());    
}

template <>
bool eval_guard(const std::initializer_list<const char*>& guard, 
                const std::string& x)
{
  return (std::find(guard.begin(),guard.end(),x)!=guard.end());    
}
